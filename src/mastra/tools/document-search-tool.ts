...
console.log("✅ Pinecone tool initialized successfully");
...
try {
  ...
  const indexName = process.env.PINECONE_INDEX_NAME || 'berkshire-letters';
...
export const documentSearchTool = createTool({
  ...
  execute: async ({ context }: { context: SearchToolContext }) => {
    try {
      const { query, topK = 5, yearFilter, minScore = 0.7 } = context;
      ...
      return {
        results: [],
        totalResults: 0,
        searchQuery: context.query || 'Unknown query',
      };
    }
  },
});